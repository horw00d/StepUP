    ...
    for root, dirs, files in os.walk(DATA_ROOT):
        if "metadata.csv" in files:
            df_steps = pd.read_csv(
                os.path.join(root, "metadata.csv"), skipinitialspace=True
            )
                ...
                with np.load(npz_path) as data:
                    ...
                    for _, row in df_steps.iterrows():
                        ...
                        try:
                            ...
                            print(
                                f"Warning: Could not process tensor {idx} in {npz_path}: {e}"
                            )
                        ...
                        step = Footstep(
                            ...
                            stance_duration_frames=(
                                physics_data["stance_duration_frames"]
                                if physics_data
                                else None
                            ),
                            time_pct_array=(
                                physics_data["time_pct_array"] if physics_data else None
                            ),
                            grf_array=(
                                physics_data["grf_array"] if physics_data else None
                            ),
                            cop_ml_array=(
                                physics_data["cop_ml_array"] if physics_data else None
                            ),
                            cop_ap_array=(
                                physics_data["cop_ap_array"] if physics_data else None
                            ),
                        )
            ...
            logging.info(
                f"Processed Trial: {participant_id} - {footwear_cond} - {speed_cond} ({len(footsteps_batch)} steps)"
            )
