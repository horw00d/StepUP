...
def fetch_trial_data(part, shoe, speed):
    ...
    with Session(engine) as session:
        ...
        stmt = select(Trial).where(
            Trial.participant_id == part, Trial.footwear == shoe, Trial.speed == speed
        )
        ...
        stmt = (
            ...
            .where(Footstep.trial_id == trial.id)
            .order_by(Footstep.footstep_index)
        )
...
def fetch_pass_options(part, shoe, speed):
    ...
    unique_passes = sorted(
        list(set([s.pass_id for s in steps if s.pass_id is not None]))
    )
...
def cached_aggregate_waveforms(step_ids_tuple):
    ...
    with Session(engine) as session:
        ...
        for i in range(0, len(step_ids_tuple), chunk_size):
            ...
            stmt = select(Footstep.time_pct_array, Footstep.grf_array).where(
                Footstep.id.in_(chunk)
            )
            ...
            for row in results:
                if time_pct is None and row.time_pct_array:
                    time_pct = (
                        row.time_pct_array
                        if isinstance(row.time_pct_array, list)
                        else json.loads(row.time_pct_array)
                    )

                if row.grf_array:
                    grf = (
                        row.grf_array
                        if isinstance(row.grf_array, list)
                        else json.loads(row.grf_array)
                    )
                    grf_list.append(grf)
